...
def load_box_scores():
    ...
    # Verify range
    min_date = con.execute("SELECT MIN(gameDateTimeEst) FROM player_box_scores").fetchone()[0]
    max_date = con.execute("SELECT MAX(gameDateTimeEst) FROM player_box_scores").fetchone()[0]
