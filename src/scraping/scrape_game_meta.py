...
TEAM_HISTORIES_PATH = "data/raw/TeamHistories.csv"
...
def load_team_map():
    ...
    """
    team_map = {}
    if not os.path.exists(TEAM_HISTORIES_PATH):
        print(
            f"Warning: {TEAM_HISTORIES_PATH} not found. Team mapping will be limited."
        )
        return team_map

    with open(TEAM_HISTORIES_PATH, "r", encoding="utf-8") as f:
...
def get_nba_game_id(con, date_str, home_team_abbr, team_map):
    ...
    if not home_team_id:
        # Try to find by city/name if abbrev fails?
        # For now, just return None
        return None

    # Query DB
    # We look for games on that date where home team matches
    # Note: gameDateTimeEst includes time, so we use LIKE or date truncation
    query = """
        SELECT gameId 
        FROM games 
        WHERE strftime(gameDateTimeEst, '%Y-%m-%d') = ? 
        AND hometeamId = ?
    """
...
def process_box_score(con, url, game_id, dry_run):
    try:
        ...
        coach_links = soup.find_all("a", href=re.compile(r"/coaches/[a-z0-9]+\.html"))
