...
def init_dimensions():
    ...
    print("Initializing dim_teams...")
    con.execute(f"""
        CREATE OR REPLACE TABLE dim_teams AS
        SELECT
    ...
    team_count = con.execute("SELECT COUNT(*) FROM dim_teams").fetchone()[0]
    print(f"Loaded {team_count} rows into dim_teams.")

    print("Initializing dim_seasons...")
    seasons = []
    ...
    con.execute("CREATE OR REPLACE TABLE dim_seasons AS SELECT * FROM df_seasons")

    season_count = con.execute("SELECT COUNT(*) FROM dim_seasons").fetchone()[0]
    print(f"Loaded {season_count} rows into dim_seasons.")
